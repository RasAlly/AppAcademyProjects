...
VALUES
  ( (SELECT id FROM questions WHERE title = 'Help for problem' ), (SELECT id FROM users WHERE fname = 'Sammy'), 'Make sure to use semicolon', NULL ),
  ( (SELECT id FROM questions WHERE title = 'Need help signing up' ), (SELECT id FROM users WHERE fname = 'John'), 'Please use the link provided in your email', NULL ),
...
INSERT INTO 
  question_likes (user_id, user_question, user_like)
VALUES
  ( (SELECT id FROM users WHERE fname = 'John'), (SELECT id FROM questions WHERE title = 'Need help signing up'), 1 ),
  ( (SELECT id FROM users WHERE fname = 'Sammy'), (SELECT id FROM questions WHERE title = 'Help for problem'), 1 );